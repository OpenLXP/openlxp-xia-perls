...
class App {
  ...
  async run() {
    ...
    while (url) {
      ...
      this.outputToConsole(jsonApiData)
      this.outputToConsole(jsonApiData.data)
      const destinationData: MappedItem = await this.transformSourceData(jsonApiData.data, transformationMapping, {});
      this.outputToConsole(destinationData)
      ...
      this.outputToConsole(values.length)
      this.outputToConsole(values)
